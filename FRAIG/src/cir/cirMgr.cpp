...
CirMgr::printNetlist() //const
{
  ...
  */
  //cout << "PRINT NET LIST" << endl;
  DFSlistbuild();
  GateList finalDFSlist;
  //cout << "SUCCESS BUILDING DFS" << endl;
  for (size_t i = 0; i < DFSlist.size(); i++) {
    if(DFSlist[i]->getTypeStr() == "UNDEF") continue;
    GateList::iterator it;
    it = find(finalDFSlist.begin(), finalDFSlist.end(), DFSlist[i]);
    if(it == finalDFSlist.end())
      finalDFSlist.push_back(DFSlist[i]);
  }
  ...
  for (size_t i = 0; i < finalDFSlist.size(); i++) {
    if(finalDFSlist[i]->getTypeStr() == "CONST") cout << "[0] CONST0" << endl;
    else{
    cout << "[" << i << "] " << setw(3) << left << finalDFSlist[i]->getTypeStr() << " " << finalDFSlist[i]->getID();
    for (size_t j = 0; j < finalDFSlist[i]->fanin.size(); j++) {
      cout << " ";
      if (finalDFSlist[i]->fanin[j]->getTypeStr() == "UNDEF") cout << "*";
      if (finalDFSlist[i]->invert_or_not[j] == false) cout << "!";
      cout << finalDFSlist[i]->fanin[j]->getID();// << " ";
    }
    if (finalDFSlist[i]->getSymbol() != "") cout << " (" << finalDFSlist[i]->getSymbol() << ")";
    cout << endl;
    }
  }
}
...
CirMgr::DFSTraversal(CirGate* travGate)
{
  ...
  if(travGate->fanin.size() == 0) { DFSlist.push_back(travGate); return;}
  else{
  ...
  if (travGate->getTypeStr() == "AIG" && travGate->fanin.size() == 2) {trueAIG.push_back(travGate->getID());}
  if (travGate->getTypeStr() == "PI") {truePI.push_back(travGate->getID());}
  if (travGate->getTypeStr() == "PO") {truePO.push_back(travGate->getID());}
  ...
  for (size_t i = 0; i < M + O + 1; i++) {
    if (glist[i] != 0) {
      if (glist[i]->getTypeStr() == "AIG") {
        if (glist[i]->fanin.size() != 2)  {glist[i]->float_or_not = true; have_fl = true;}
    ...
    for (size_t i = 0; i < glist.size(); i++)
      if (glist[i] != 0)
        if (glist[i]->def_or_not == true) {
          cout << " " << glist[i]->getID();
          //cout << " " << glist[i]->fanin.size() << " " << glist[i]->fanout.size() << endl;
        }
    cout << endl;
  }
}
...
{
  for (unsigned n = 0; n < FEClist.size(); ++n) {
    cout << "[" << n << "]";
    for (unsigned m = 0; m < FEClist[n].size(); ++m)
      cout << " " << ((FEClist[n])[m] % 2 ? "!" : "") << (FEClist[n])[m] / 2;
    cout << endl;
  }
}
...
CirMgr::writeAag(ostream& outfile) //const
{
  ...
  finalPO.clear();
  finalPO.push_back(truePO[0]);
  //outfile << "truePO size = " << truePO.size() << endl;
  //outfile << "truePO[0] = " << glist[truePO[0]]->fanin[0]->getID()*2 << endl;
  for (size_t i = 0; i < truePO.size(); i++){
    unsigned t = truePO[i]; //t = ID
    vector<unsigned>::iterator it;
    it = find(finalPO.begin(), finalPO.end(), t);
    if(it == finalPO.end()){
      finalPO.push_back(t);
      //outfile << "push" << t << endl;
    }
  }
  for (size_t i = 0; i < finalPO.size(); i++){
      outfile << glist[finalPO[i]]->fanin[0]->getID()*2 + (glist[finalPO[i]]-> invert_or_not[0] == 0) << endl;
  }
  //outfile << "trueAIG size = " < endl;
  for (size_t i = 0; i < trueAIG.size(); i++) {
    unsigned t = trueAIG[i];
    outfile << glist[t] -> op << " " << glist[t]->fanin[0]->getID()*2 + (glist[t]-> invert_or_not[0] == 0) 
                              << " " << glist[t]->fanin[1]->getID()*2 + (glist[t]-> invert_or_not[1] == 0) << endl;
  }
  for (size_t i = 0; i < I; i++)
      ...
      for (size_t j = 0; j < 2; j++) {
        if (glist[i] -> getTypeStr() != "PI" && !(j == 1 && glist[i] -> getTypeStr() == "PO") && glist[i] -> in[j] / 2 == 0)
          ...
          connectCircuit(glist[(glist[i]->in[j]) / 2], glist[i], j);
                 ...
                 glist[i] -> getTypeStr() != "PI" && glist[i] -> getTypeStr() != "UNDEF") { //undef id(in[0] in[1]) doesnt in gatelist
          glist[(glist[i]->in[j]) / 2] = new UNDEFGate((glist[i]->in[j]) / 2 , 0);
          glist[glist[i]->in[j] / 2]-> op = glist[i]->in[j];
          glist[glist[i]->in[j] / 2]-> in[0] = -1;
          glist[glist[i]->in[j] / 2]-> in[1] = -1; 
          // for strash05
          connectCircuit(glist[glist[i]->in[j] / 2], glist[i], j);
          /*glist[glist[i]->in[j] / 2]->fanin[0] = 0;
          glist[glist[i]->in[j] / 2]->fanin[1] = 0;
          glist[glist[i]->in[j] / 2]->invert_or_not[0] = 1;
          glist[glist[i]->in[j] / 2]->invert_or_not[1] = 1;
          glist[glist[i]->in[j] / 2]->fanout.push_back(glist[i]);
          */
        }
      }
    }
