...
TaskMgr::add(const string& s, size_t l)
{
  ...
  if(!_taskHash.insert(newNode)){
    //cout << "Error: Task node (" << s << ") already exists." << endl;
    return false;  
  }
  _taskHeap.insert(newNode); _taskHash.insert(newNode);
  cout << "Task node inserted: " << newNode << endl;
...
TaskMgr::assign(size_t l)
{
  ...
  TaskNode newNode(min().getName(), min().getLoad() + l);
  _taskHeap.delMin();
  _taskHeap.insert(newNode);
  
  //_taskHash.query(newNode);
