...
DBAppendCmd::exec(const string& option)
{
   ...
   //assert( CmdExec::lexOptions(option, options, 0) );
   ...
   bool doRow = false ;
   if (myStrNCmp("-Row", options[0], 2) == 0) doRow = true ; //2: -R
   ...
   for(int i  = 1; i < options.size(); i++){
      if(options[i] == "-")   
         ...
         cout << "Error: not an available number! " << endl;
   ...
   if (doRow) {
      ...
      num.resize(dbtbl.nCols(), NOO);
      DBRow newrow(num);
      dbtbl.addRow(newrow);
   }      

   else {
      num.resize(dbtbl.nRows(), NOO);
         dbtbl.addCol(num);
   }
...
{  
   ...
   //else 
      if (myStrNCmp("-Table", options[0], 2) == 0){
      if(options.size() != 1) return CMD_EXEC_ERROR;
      else cout << dbtbl << endl;
   }
   ...
   //else if (myStrNCmp("-Summary", options[0], 2) != 0)
      ...
      if(options.size() != 1) return CmdExec::errorOption(CMD_OPT_EXTRA, options[0]);
   ...
   //else if (myStrNCmp("-Row", options[0], 2) != 0)
      ...
      if (options.size() != 2 || (!checkRowIdx(options[1],c))) return CMD_EXEC_ERROR;
      //else dbtbl.printData();//
      else 
         for(int i = 0; i < dbtbl.nCols(); i++){ 
            if(dbtbl[c][i] == NOO) cout << setw(6) << right << ".";
            else cout << setw(6) << right << dbtbl[c][i];}
   ...
   //if (myStrNCmp("-Column", options[0], 2) != 0)
      ...
      if (options.size() != 2 || (!checkColIdx(options[1],d))) return CMD_EXEC_ERROR;

      else dbtbl.printCol(d);
   }

   else if(options.size() == 2 && (checkRowIdx(options[0], c)) && (checkColIdx(options[1], d))){
      if(dbtbl[c][d] == NOO) cout << "Null";
         else cout << setw(6) << right << dbtbl[c][d];
   }
   

   else //{cout << "Error: Missing option! " << endl; }
      return CMD_EXEC_ERROR;
