...
CmdParser::openDofile(const string& dof)
{
   ...
   _dofile = new ifstream(dof.c_str());//_dofile is *istream
...
CmdParser::closeDofile()
{
  ...
  if(!_dofileStack.empty()){
      ...
      _dofile = _dofileStack.top();
      _dofileStack.pop();//euqal to delete
      
      //else
      //   _dofile = 0;
  }
   else _dofile = 0;// = delete _dofile; 
}
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
   size_t firstword = str.find_first_not_of(' ');
   size_t secondblank = str.find(' ', firstword);//比較像是1st word end

   string cmd = str.substr(firstword, (secondblank - firstword));
   ...
   if(e == NULL){
      ...
      if(secondblank > str.size())  option = "";//no option
      else option = str.substr(secondblank + 1);//from that pos to end
   }
   return e;
}
...
CmdParser::listCmd(const string& str)
{
   ...
   //keystr = keystr.substr(0, _readBufPtr - _readBuf);//str after cut, b4 cursor
   
   size_t firstword = str.find_first_not_of(' ',0);
   
   string keystr;
   if(firstword != string::npos)
      keystr = str.substr(firstword);
   

   string token;
   size_t endpos = myStrGetTok(str,token);
   ...
   if(token == ""){
        int i = 0;
        cout << endl;
   //if (firstword == string::npos){//npos = -1, no matches : blank
      for (it = a.begin(); it != a.end(); it++){
   ...
   //multi match
      ...
      //cerr << "keke" << endl;
      std::vector<string> matchcmd_stored;
     //if(string::npos == endpos){
      for(it = a.begin(); it != a.end(); it++){
         string mapping = it -> first + it -> second -> getOptCmd();
         if(token.size() > mapping.size()) continue;
         if(myStrNCmp(mapping, token, token.size()) == 0){
            //cerr << mapping  << " "<<  token << " " << it->first.size() << endl;
            matchcmd_stored.push_back(mapping);
         }
      }
      
      if(matchcmd_stored.empty()){ 
         //cerr << "keke" << endl;
         mybeep();}
      else if(matchcmd_stored.size() == 1){

          for (int i = str.size(); i < matchcmd_stored[0].size(); ++i)// i is a ptr.
            insertChar(matchcmd_stored[0][i]);
         insertChar(' ');
      }
         ...
         cout << endl;
         //cerr << "lol" << endl;
         for(size_t i = 0 ; i < matchcmd_stored.size(); i++){
            if(i % 5 == 0) cout << endl;
            cout << setw(12) << left << matchcmd_stored[i];
         }
         reprintCmd();      
      }
   }
...
CmdParser::getCmd(string cmd)
{
   ...
   CmdMap::iterator it;
   CmdMap a = _cmdMap;
   for(it = a.begin(); it != a.end(); it++){// a = _cmdMap?
      if(cmd.length() >= it->first.size()){
         //cout << it->first << " " << myStrNCmp(cmd, (it -> first) + (it -> second -> getOptCmd()), it->first.size()) << endl;
         if(!myStrNCmp(cmd, it -> first , it->first.size()))
            e = it -> second;
      }   
   }
      return e;
